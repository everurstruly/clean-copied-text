...
- **`lib/cleaner.ts`**: The core text processing engine. It takes the raw text and user-selected options, and applies a series of regex and string manipulation rules to clean and format the text.
...
4. **Text Processing**: The text and options are sent to `cleanText()`, which applies the selected rules sequentially.
